...
Johnson, et al.~\citep{RN33} showed significant increases in adaptation rate with increased mean error and decreased feedback uncertainty.
...
Lyons and Joshi~\citep{RN35} demonstrated that subjects exposed to a mapping uncertainty during cursor control had larger adaptation rates when the mapping uncertainty was removed, indicating that noise during training may increase adaptation.
...
While there have been many proposed models for trust, Hoff and Bashir's three layer model deserves particular attention~\citep{RN39}.
...
The experimental setup followed Lyons and Joshi~\citep{RN44}.
...
The electrodes' signal was acquired as described in Ref.~\citep{RN44}, and the signal processing followed Ref.~\citep{RN45}.
...
The Command Accuracy Test was conducted prior to training, at the end of training, and at the end of retention to provide an assessment similar to offline classification accuracy (see Ref.~\citep{RN48} for an EMG example).
...
The maximum Trial time was 60 s, which was determined by reviewing data from Ref.~\citep{RN45} and preliminary testing.
...
For example, Braun, et al.~\citep{RN36} showed that subjects trained with an adaptive training strategy were able to quickly generalize to novel tasks with similar underlying structure.
...
The Concurrent Feedback and Terminal Feedback groups follow the findings of Basmajian~\citep{RN27} that augmented feedback can improve performance.
We also observed effects that may be explained by Hoff and Bashir's three layer model~\citep{RN39}.
