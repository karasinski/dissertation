...
$\zeta_{NM}$ also varied significantly as a result of training, but further investigating showed that the effects of varying this parameter did not significantly effect the resulting outputs of the model.
...
As a result of the analysis resulting from the linear mixed effects models, we determined that all of the parameters can be treated as constants with the exception of $K_e$, which changes with both Trial and Group.
The resulting values for the remainder of the parameters were identified as the values listed in Table~\ref{table:structuralmodelresultants}, see below.
\begin{table}[tb]
    \centering
...
\end{table}
The final parameter, $K_e$, can be modeled as an exponential function that increases with training.
...
An extension to the Structural Model was proposed which can explain the effects observed when operators were exposed to concurrent bandwidth feedback.
