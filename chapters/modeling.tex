...
For the Pitch only control mode, the display chagned from yellow to red when the pitch deviated by greater than three degrees.
...
This task dynmaics describe a Boeing 747 in Flight Condition 2, and resultant elevator to pitch dynamics take the form~\citep{heffley1972aircraft}:
\begin{align}
...
A four step process developed to tackle the problem of finding the optimal model parameters, the results of which were compared to the results of the ARX technique to validate our approach.
The general outline of the technique was:
\begin{enumerate}
    \item A brute force parameter search over the six remaining model parameters.
...
The first step of the value identification technique was a brute force parameter search used to identify the value of the remaining parameters, see Table~\ref{table:structuralmodelparameters}.
Each combination of the parameters $K_e, \tau_0, K, a$, $\omega_{NM}$ and $\zeta_{NM}$ were enumerated over and simulated in Simulink using the same flight model and disturbance profiles as the participants experienced in the experiment.
...
By investigating the control mode with the smallest effect of the feedback, we can show that this technique is sensitive enough to detect the smallest changes between the two groups.
